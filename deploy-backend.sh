...
echo "=== Deploying Backend with Schema Initialization ==="
...
ECR_REPO="337756366856.dkr.ecr.us-east-1.amazonaws.com/trucking-tms-backend"
...
$DOCKER build -t trucking-tms-backend:latest backend/

echo "Step 3: Tagging image..."
$DOCKER tag trucking-tms-backend:latest $ECR_REPO:latest
...
$AWS ecs update-service \
  ...
  --service trucking-tms-backend-service \
  --force-new-deployment \
  --region us-east-1

echo ""
echo "=== Deployment Complete! ==="
echo "The backend will be updated in ~2-3 minutes."
echo "Check status: $AWS ecs describe-services --cluster trucking-tms-cluster --services trucking-tms-backend-service --region us-east-1"
